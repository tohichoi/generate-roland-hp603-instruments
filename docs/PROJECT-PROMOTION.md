...
`reference/`에 커밋된 PDF 8건은 모두 제3자 저작물입니다. 실제 구성은 **Roland 문서 7건**(MIDI 구현·음색 목록·Fantom/SRX 선택 가이드)과 **Cakewalk 언어 가이드 1건**이며, Yamaha 문서는 없습니다.

이 구성은 감사에서 뜻밖의 비대칭을 드러냈습니다. Roland의 318건 전사는 `Midi_Implementatie_Roland_LX-7.pdf`가 저장소에 있어 원문 대조가 가능하지만, **Yamaha 511건의 1차 자료인 CLP-685/695GP Data List는 저장소에 없어 전사 정확성을 원문과 대조할 수 없습니다.** 이 보고서의 Yamaha 수치는 전사본(`data/yamaha-clp685-data.txt`)을 기준으로 검증한 것이며, 제조사 원문과의 일치 여부는 확인 범위 밖입니다.

공개 저장소에 제3자 문서 원문을 두는 것은 재배포 허가 범위 밖일 수 있습니다.
